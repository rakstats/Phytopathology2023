################################################################################
# This Script generates the Variogram plots of Field-W for the paper. The      #
...
################################################################################
#                                                                              #  
# a. Paddock = W; variable = dPCR; Sampling design = "b" (rake and 1.2 m2)     #
#                                                                              #  
################################################################################
...
padd_W_1.2sqm_vgram_cloud_plt2 <- padd_W_1.2sqm_vgram_cloud_plt +
  ggtitle(bquote(bold(atop("(a) Field-SD-a, 1.2"~m^ 2~"(r)")))) +
...
################################################################################
padd_W_1.2sqm_dPCR_vgram_jointplt <- (padd_W_1.2sqm_vgram_cloud_plt2 / 
                                        padd_W_1.2sqm_dpcr_vgram_plt)
################################################################################


################################################################################
#                                                                              #  
# b. Paddock = W; variable = dPCR; Sampling design = "c" (rake and 8.6 m2)     #
#                                                                              #  
################################################################################
# Filter out the necessary data
...
padd_W_8.6sqm_vgram_cloud_plt2 <- padd_W_8.6sqm_vgram_cloud_plt +
  ggtitle(bquote(bold(atop("(b) Field-SD-a, 8.6"~m^ 2~"(r)")))) +
  theme(plot.title = element_text(size = 15, face = "bold"))
################################################################################
# Construct the variogram                                                      #
################################################################################
...
padd_W_8.6sqm_dpcr_vgram_plt <- padd_W_8.6sqm_low_and_upplim_data %>% 
  ...
  xlim(0, 350)
################################################################################
padd_W_8.6sqm_dPCR_vgram_jointplt <- (padd_W_8.6sqm_vgram_cloud_plt2 / 
                                        padd_W_8.6sqm_dpcr_vgram_plt)
################################################################################
...
################################################################################
#                                                                              #  
# c. Paddock = W; variable = dPCR; Sampling design = "d" (rake and 60 m2)      #
#                                                                              #  
################################################################################
# Filter out the necessary data
...
padd_W_60sqm_vgram_cloud_plt2 <- padd_W_60sqm_vgram_cloud_plt +
  ggtitle(bquote(bold(atop("(c) Field-SD-a, 60"~m^ 2~"(r)")))) +
  theme(plot.title = element_text(size = 15, face = "bold"))
################################################################################
# Construct the variogram                                                      #
################################################################################
...
################################################################################
padd_W_60sqm_dPCR_vgram_jointplt <- (padd_W_60sqm_vgram_cloud_plt2 / 
                                       padd_W_60sqm_dpcr_vgram_plt)
################################################################################

################################################################################
#                                                                              #
#       Field-W Variogram Plot for Paper                                       #
#                                                                              #
################################################################################
padd_W_dPCR_vgram_jointplt <- (padd_W_1.2sqm_dPCR_vgram_jointplt |
                               padd_W_8.6sqm_dPCR_vgram_jointplt |
                               padd_W_60sqm_dPCR_vgram_jointplt)

ggsave(here::here("Figures", 
                  "Field_W_VariogramPlots.jpeg"),
       padd_W_dPCR_vgram_jointplt, height = 9, width = 15)

ggsave(here::here("Figures", 
                  "Field_W_VariogramPlots.png"),
       padd_W_dPCR_vgram_jointplt, height = 9, width = 15)

ggsave(here::here("Figures", 
                  "Field_W_VariogramPlots.tiff"),
       padd_W_dPCR_vgram_jointplt, height = 9, width = 15)
